  ...
  subjects: [
    {
      ...
      days: [
        ...
        { day: 'Wednesday', date: '2026-07-15', topic: 'Rational Functions', prepared: false, missingItems: ['Activity Sheet', 'Lecture Content'] },
        { day: 'Friday', date: '2026-07-17', topic: 'Exponential Functions', prepared: false, missingItems: ['Lesson Plan', 'Assessment', 'Lecture Content'] },
      ]
    },
    {
      id: 'sub-sci-11',
...
const AIPanel = ({ 
  ...
  // UX Best Practice: Pass the context to the next page so the user doesn't have to re-select it
  const aiToolsUrl = `/dashboard/ai-tools?target=${encodeURIComponent(missingItem)}&topic=${encodeURIComponent(day.topic)}&subject=${encodeURIComponent(subject.name)}`;

  return (
    <div className="h-full flex flex-col bg-white/60 dark:bg-[#121212]/80 backdrop-blur-xl border-l border-slate-200/60 dark:border-white/10 shadow-xl overflow-hidden animate-in slide-in-from-right-8 duration-300">
      {/* Panel Header */}
      <div className="flex items-center justify-between p-4 border-b border-slate-200/60 dark:border-white/10 bg-white/50 dark:bg-transparent">
        <div>
            ...
            <p className="text-[12px] text-slate-500 dark:text-slate-400 mt-0.5">Missing material for {day.day}</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 custom-scrollbar relative">
        <div className="mb-8">
            <h4 className="text-[13px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Lesson Context</h4>
        ...
        <div className="flex flex-col items-center justify-center text-center mt-6 gap-5">
          <div className="w-16 h-16 rounded-2xl bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center border border-blue-100 dark:border-blue-800/30">
              <FileText size={28} style={{ color: theme.blue }} />
          </div>
          
          <div className="space-y-2">
            <h3 className="text-base font-bold text-slate-900 dark:text-white">
              Create your {missingItem}
            </h3>
            <p className="text-sm text-slate-600 dark:text-slate-300 max-w-[220px] mx-auto leading-relaxed">
              Head over to the dedicated AI workspace to draft, edit, and finalize this material.
            </p>
          </div>
          ...
          <div className="w-full space-y-3 mt-4">
            ...
            <a 
              ...
              className="flex items-center justify-center gap-2 w-full py-2.5 rounded-[8px] font-bold text-sm transition-all shadow-md hover:shadow-lg text-white hover:-translate-y-0.5 active:translate-y-0"
              style={{ backgroundColor: theme.blue }}
            >
              Generate {missingItem} Now
              <ExternalLink size={16} className="ml-1 opacity-90" />
            </a>

            {/* Secondary Action: Resolves item without leaving the page */}
            <button 
              onClick={() => onComplete(subject.id, day.date, missingItem)}
              className="w-full py-2 text-[13px] font-medium text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white transition-colors flex items-center justify-center gap-2"
            >
  ...
  return (
    <div className={isDarkMode ? 'dark w-full flex justify-center' : 'w-full flex justify-center'}>
       ...
       {/* Smart Alert Area */}
        <div className="flex-grow w-full md:w-auto">
          {nextAction ? (
            ...
            <div className="w-full text-left flex items-center justify-between p-4 rounded-[12px] bg-blue-50/60 dark:bg-[#0B9ECC]/10 backdrop-blur-md border border-blue-200/50 dark:border-[#0B9ECC]/20 shadow-sm animate-in fade-in zoom-in duration-300">
               <div>
      ...
      {isModalOpen && (
        ...
        >
          <div 
            className="relative w-full max-w-6xl h-[90vh] sm:h-[85vh] flex rounded-[16px] shadow-2xl overflow-hidden border bg-[#F8FAFC] dark:bg-[#0A0A0A] border-slate-300/50 dark:border-white/10"
          ...
          >
                        ...
                        {weekData.subjects.map(subject => (
                            <div key={subject.id} className="bg-white dark:bg-[#121212] rounded-[12px] border border-slate-200 dark:border-white/10 shadow-sm overflow-hidden">
                                ...
                                <div className="px-5 py-3.5 border-b border-slate-100 dark:border-white/5 flex items-center justify-between bg-slate-50/50 dark:bg-white/[0.02]">
                                    ...
                                    <div className="text-[12px] font-medium text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-white/5 px-3 py-1 rounded-full transition-all duration-300">
                                    ...
                                    {subject.days.map((day, index) => {
                                        ...
                                        return (
                                            <div 
                                                key={index}
                                                className={`
                                                    relative flex flex-col p-4 rounded-[10px] border transition-all duration-300
                                                    ${day.prepared 
                                            ...
                                            >
                                                <div className="flex justify-between items-start mb-2">
                                                    ...
                                                    {day.prepared ? (
                                                        <span className="flex items-center gap-1 text-[11px] font-bold text-[#0B9ECC] bg-[#0B9ECC]/10 px-2 py-0.5 rounded-full animate-in zoom-in duration-300">
                                                ...
                                                {!day.prepared && day.missingItems && (
                                                    <div className="mt-auto border-t border-slate-100 dark:border-white/5 pt-3 animate-in fade-in duration-300">
            ...
            {activeTask && (
                <div className="w-full lg:w-[380px] shrink-0 border-l border-slate-200 dark:border-white/10 z-20">
                    <AIPanel 
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb:hover { background-color: rgba(150, 150, 150, 0.4); }
        @media (prefers-color-scheme: dark) {
          .custom-scrollbar::-webkit-scrollbar-thumb { background-color: rgba(255, 255, 255, 0.1); }
